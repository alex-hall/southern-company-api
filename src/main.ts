...
export class SouthernCompanyAPI extends EventEmitter {
	...
	private async getScWebToken(requestVerificationToken: string, username: string, password: string) {
		...
		const resData: LoginResponse = await response.json() as LoginResponse;
	...
	public async getDailyData(startDate: Date, endDate: Date) {
		...
		return await Promise.all(responses.map(async (accountResponses, index) => {
			...
			const accountResData: DailyDataResponse[] = await Promise.all(accountResponses.map((res) => res.json() as Promise<DailyDataResponse>));
	...
	public async getMonthlyData() {
		...
		const resData: MonthlyDataResponse[] = await Promise.all(responses.map((response) => response.json() as Promise<MonthlyDataResponse>));
	...
	public async getAllBillsData() {
		...
		/* Parsing response */
		const allbills: GetAllBillsResponse = await response.json() as GetAllBillsResponse;
